...
def test_dsp_item_source_content_and_catalog_contract(
    item: dict[str, Any], catalog: CourseCatalog
) -> None:
    index = item["number"] - 1
    source = SOURCE_MAP["items"][index]
    mapped = CONVERSION_MANIFEST["items"][index]
    module_root = COURSE_ROOT / item["target_folder"]
    record = _yaml(module_root / "conversion.yaml")
    course, module = catalog.module_record("dsp-radar", item["target_module_id"])

    assert course.manifest.id == "dsp-radar"
    ...
    source_root_value = os.environ.get("ELP_DSP_SOURCE_ROOT")
...
@pytest.mark.parametrize("item", SELECTED, ids=lambda item: item["id"])
def test_dsp_item_deterministic_runtime_sweeps_and_failure_recovery(
    ...
    first = runtime.run("dsp-radar", item["target_module_id"], {}).model_dump(mode="json")
    second = runtime.run("dsp-radar", item["target_module_id"], {}).model_dump(mode="json")
    ...
    for sweep in record["content"]["sweeps"][:2]:
        ...
        values = sweep["values"]
        low = runtime.run(
            "dsp-radar", item["target_module_id"], {control.id: values[0]}
        ).model_dump(mode="json")
        high = runtime.run(
            "dsp-radar", item["target_module_id"], {control.id: values[-1]}
        ).model_dump(mode="json")
...
@pytest.mark.parametrize("item", SELECTED, ids=lambda item: item["id"])
def test_dsp_item_retained_numeric_equivalence(item: dict[str, Any]) -> None:
    ...
    for case in equivalence["cases"]:
        ...
        actual = _numeric_leaves(json.loads(actual_path.read_text(encoding="utf-8")))
