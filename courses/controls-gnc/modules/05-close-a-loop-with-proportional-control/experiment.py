...
ITEM_NUMBER = 5
BROKEN_TEXT = (
    "The broken toggle reverses the measurement sign, producing positive feedback."
)
RECOVERY_TEXT = (
    "Disable the broken case to restore subtraction at the summing junction."
)

PLOT_SPECS = {
    "response": {
        "title": "Proportional closed-loop response",
    ...
    "mechanism": {
        "title": "Control action and tracking error",
        "axes": {
            "x": "Time (s)",
            "y": "Normalized control command (ratio)",
            "y2": "Tracking error (ratio)",
        },
        "traces": [
            {
                "x_quantity": "Time",
...
def _simulate(parameters: dict[str, Any], broken_mode: bool) -> dict[str, Any]:
    kp = float(parameters["proportional_gain"])
    tau = float(parameters["plant_time_constant_s"])
    sign = 1.0 if broken_mode else -1.0
    dt = 0.01
    ...
    for i in range(len(t) - 1):
        u[i] = kp * (1 + sign * y[i])
        y[i + 1] = y[i] + dt * (-y[i] + u[i]) / tau
    u[-1] = kp * (1 + sign * y[-1])
    stable = 1 + kp if sign < 0 else 1 - kp
    return result(
        broken_mode,
        t,
        [trace("Output", t, y), trace("Reference", t, np.ones_like(t), "dash")],
        [trace("Control effort", t, u), trace("Tracking error", t, 1 - y)],
        [
            ("final_error", "Final tracking error", 1 - y[-1], "output"),
            ("closed_loop_rate", "Signed loop rate", stable / tau, "1/s"),
            ("peak_control", "Peak control", np.max(np.abs(u)), "actuator"),
        ],
        [kp, tau, sign, y[-1], np.max(np.abs(y))],
        "Negative proportional feedback changes both response rate and equilibrium; the summing-junction sign is part of the plant model.",
    )
