# Recover a Trajectory with Rauch-Tung-Striebel Smoothing

**Guiding question:** Why can a backward smoothing pass reduce past uncertainty after future measurements arrive?

Compare scalar filtered and RTS-smoothed covariance and quantify the lagged improvement without calling it real-time estimation. This is a Python-first native design derived from the reviewed competency map. It is not presented as a conversion of the pinned MATLAB source and it stays inside deterministic software simulation.
...
$$\text{C_k=P_k F^T(P^-_{k+1})^-1}$$
$$\text{x^s_k=x_k+C_k(x^s_{k+1}-x^-_{k+1})}$$
$$\text{P^s_k=P_k+C_k(P^s_{k+1}-P^-_{k+1})C_k^T}$$
...
RTS smoothed covariance is no larger than filtered covariance for the same linear-Gaussian model and complete future data. State which output should move first and which quantity should remain invariant before changing a control.
...
1. Hold `measurement_variance` at `0.4 1` and sweep `process_variance` from `0.001` through `0.08` to `0.5 1`.
2. Restore `process_variance` to `0.08 1` and sweep `measurement_variance` from `0.02` through `0.4` to `2.0 1`.
...
Broken mode skips the backward pass, so smoothed and filtered uncertainty are identical. Broken mode is a named counterexample, not an alternative design recommendation.

## Recovery

Restore the backward recursion after the forward filter completes and label the result offline, not causal. The recovery case restores the exact baseline inputs so the evidence can prove that the failure is reversible rather than merely different.

## Limiting cases and invariants

- At the final sample smoothed and filtered estimates coincide.
- With no future information the smoother cannot reduce uncertainty.

Teaching invariant: RTS smoothed covariance is no larger than filtered covariance for the same linear-Gaussian model and complete future data.
...
Derive one signature quantity from the displayed equations, explain what the broken case violates, and name one result that this lab cannot establish. Then describe how the two sweeps separate sensitivity to `process_variance` from sensitivity to `measurement_variance`.
