# Compare Impedance and Admittance Control

**Guiding question:** What assumptions and evidence make compare impedance and admittance control defensible?

Build a deterministic numerical laboratory to compare impedance and admittance control, expose its governing relation, and diagnose a named counterexample before recovery. This module is a Python-first native design authorized by the reviewed issue-440 competency map. It is not a conversion of the pinned MATLAB-oriented source course, and it remains deterministic software evidence.
...
- $$F=K_v(x_d-x)-B_v x_dot$$
- $$M_a x_ddot+B_a x_dot+K_a x=F_ext$$
- $$E=1/2 K_v e^2$$
...
Impedance shapes force from motion error while admittance shapes motion from force; both require an energy and damping interpretation at contact. Predict the sign and direction of all three signature changes before moving a slider. Identify the equation term responsible and one quantity that should remain invariant. This written prediction is the comparison point; post-hoc description is not the same as a test.
...
1. Hold `virtual_damping_n_s_m` at `45.0 N*s/m` and sweep `environment_stiffness_n_m` from `50.0` through `800.0` to `3000.0 N/m`.
2. Restore `environment_stiffness_n_m` to `800.0 N/m` and sweep `virtual_damping_n_s_m` from `1.0` through `45.0` to `150.0 N*s/m`.
...
Broken mode uses negative virtual damping, injecting energy into the contact oscillation. Broken mode is a falsifying counterexample, not a recommended alternative. Explain which assumption is violated before describing the visual symptom. Then locate the first intermediate quantity that departs from the baseline invariant; downstream task error alone rarely identifies the cause.

## Recovery

Restore positive damping, select architecture from actuator/sensor causality, and verify decaying stored energy. Recovery is complete only when the original default inputs and diagnostic signature return within the independent-reference tolerance. A different setting that happens to look better is mitigation, not recovery. Preserve the fault, detection, decision, and recovery sequence as separate evidence.

## Alternative and limiting cases

- In free space, environment contact force is zero.
- With very large environment stiffness, small penetration creates large force and demands adequate damping.
