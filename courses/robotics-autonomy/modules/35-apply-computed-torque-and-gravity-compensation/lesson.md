# Apply Computed Torque and Gravity Compensation

**Guiding question:** What assumptions and evidence make apply computed torque and gravity compensation defensible?

Build a deterministic numerical laboratory to apply computed torque and gravity compensation, expose its governing relation, and diagnose a named counterexample before recovery. This module is a Python-first native design authorized by the reviewed issue-440 competency map. It is not a conversion of the pinned MATLAB-oriented source course, and it remains deterministic software evidence.
...
- $$tau=M(q)v+C q_dot+g$$
- $$v=q_ddot_d-K_d e_dot-K_p e$$
- $$e_ddot+K_d e_dot+K_p e=Delta$$
...
Computed torque yields the selected nominal error dynamics only to the extent that inertia, velocity, and gravity models cancel consistently. Predict the sign and direction of all three signature changes before moving a slider. Identify the equation term responsible and one quantity that should remain invariant. This written prediction is the comparison point; post-hoc description is not the same as a test.
...
1. Hold `tracking_bandwidth_per_s` at `3.0 1/s` and sweep `model_error_fraction` from `0.0` through `0.08` to `0.6 1`.
2. Restore `model_error_fraction` to `0.08 1` and sweep `tracking_bandwidth_per_s` from `0.5` through `3.0` to `8.0 1/s`.
...
Broken mode reverses gravity compensation, doubling the configuration-dependent load. Broken mode is a falsifying counterexample, not a recommended alternative. Explain which assumption is violated before describing the visual symptom. Then locate the first intermediate quantity that departs from the baseline invariant; downstream task error alone rarely identifies the cause.

## Recovery

Restore gravity sign, bound model mismatch, and verify both tracking and actuator torque authority. Recovery is complete only when the original default inputs and diagnostic signature return within the independent-reference tolerance. A different setting that happens to look better is mitigation, not recovery. Preserve the fault, detection, decision, and recovery sequence as separate evidence.

## Alternative and limiting cases

- With exact model and zero error, commanded torque equals feedforward dynamics.
- At zero gravity, gravity compensation contributes no torque.
