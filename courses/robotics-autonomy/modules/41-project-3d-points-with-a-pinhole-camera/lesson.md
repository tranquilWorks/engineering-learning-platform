...
**Guiding question:** What assumptions and evidence make project 3d points with a pinhole camera defensible?

Build a deterministic numerical laboratory to project 3d points with a pinhole camera, expose its governing relation, and diagnose a named counterexample before recovery. This module is a Python-first native design authorized by the reviewed issue-440 competency map. It is not a conversion of the pinned MATLAB-oriented source course, and it remains deterministic software evidence.
...
- `image_radius` (px)
- `depth_sensitivity` (px/m)
- `behind_camera_count` (count)
...
Perspective projection is defined only for positive camera-frame depth and its pixel sensitivity grows inversely with depth. Predict the sign and direction of all three signature changes before moving a slider. Identify the equation term responsible and one quantity that should remain invariant. This written prediction is the comparison point; post-hoc description is not the same as a test.
...
1. Hold `point_depth_m` at `3.0 m` and sweep `focal_length_px` from `150.0` through `520.0` to `1200.0 px`.
2. Restore `focal_length_px` to `520.0 px` and sweep `point_depth_m` from `0.3` through `3.0` to `12.0 m`.
...
Broken mode uses world-frame depth without applying the camera pose and projects a point behind the camera. Broken mode is a falsifying counterexample, not a recommended alternative. Explain which assumption is violated before describing the visual symptom. Then locate the first intermediate quantity that departs from the baseline invariant; downstream task error alone rarely identifies the cause.

## Recovery

Transform every point into the camera frame, reject nonpositive depth, and verify projection against a ray ratio. Recovery is complete only when the original default inputs and diagnostic signature return within the independent-reference tolerance. A different setting that happens to look better is mitigation, not recovery. Preserve the fault, detection, decision, and recovery sequence as separate evidence.

## Alternative and limiting cases

- On the optical axis, the image point equals the principal point.
- As depth grows, pixel displacement from the principal point tends to zero.
