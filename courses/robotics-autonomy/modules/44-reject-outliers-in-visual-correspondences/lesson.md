# Reject Outliers in Visual Correspondences

**Guiding question:** What assumptions and evidence make reject outliers in visual correspondences defensible?

Build a deterministic numerical laboratory to reject outliers in visual correspondences, expose its governing relation, and diagnose a named counterexample before recovery. This module is a Python-first native design authorized by the reviewed issue-440 competency map. It is not a conversion of the pinned MATLAB-oriented source course, and it remains deterministic software evidence.
...
Robust correspondence fitting must recover a consensus model from the declared inlier ratio and verify it on residuals not used to hypothesize the model. Predict the sign and direction of all three signature changes before moving a slider. Identify the equation term responsible and one quantity that should remain invariant. This written prediction is the comparison point; post-hoc description is not the same as a test.
...
1. Hold `ransac_threshold_px` at `2.0 px` and sweep `outlier_fraction` from `0.0` through `0.35` to `0.85 1`.
2. Restore `outlier_fraction` to `0.35 1` and sweep `ransac_threshold_px` from `0.3` through `2.0` to `8.0 px`.
...
Broken mode fits all matches by least squares, allowing outliers to bias the model and pass themselves. Broken mode is a falsifying counterexample, not a recommended alternative. Explain which assumption is violated before describing the visual symptom. Then locate the first intermediate quantity that departs from the baseline invariant; downstream task error alone rarely identifies the cause.

## Recovery

Restore minimal-set sampling, residual gating, and a final refit on consensus inliers. Recovery is complete only when the original default inputs and diagnostic signature return within the independent-reference tolerance. A different setting that happens to look better is mitigation, not recovery. Preserve the fault, detection, decision, and recovery sequence as separate evidence.

## Alternative and limiting cases

- At zero outliers, robust and ordinary least squares agree in the noiseless limit.
- As the inlier ratio falls, required RANSAC trials grow rapidly.
