# Build Occupancy Grids from Range Data

**Guiding question:** What assumptions and evidence make build occupancy grids from range data defensible?

Build a deterministic numerical laboratory to build occupancy grids from range data, expose its governing relation, and diagnose a named counterexample before recovery. This module is a Python-first native design authorized by the reviewed issue-440 competency map. It is not a conversion of the pinned MATLAB-oriented source course, and it remains deterministic software evidence.
...
- $$l_t=l_{t-1}+logit(p(z|m))-l_0$$
- $$p=1/(1+exp(-l))$$
- $$inverse sensor model marks free then occupied$$
...
Log-odds mapping must distinguish traversed free cells from terminal occupied cells and avoid treating unobserved space as free. Predict the sign and direction of all three signature changes before moving a slider. Identify the equation term responsible and one quantity that should remain invariant. This written prediction is the comparison point; post-hoc description is not the same as a test.
...
1. Hold `beam_count` at `90.0 count` and sweep `hit_probability` from `0.51` through `0.72` to `0.98 1`.
2. Restore `hit_probability` to `0.72 1` and sweep `beam_count` from `8.0` through `90.0` to `360.0 count`.
...
Broken mode marks every unreturned beam cell as free, erasing unseen obstacles. Broken mode is a falsifying counterexample, not a recommended alternative. Explain which assumption is violated before describing the visual symptom. Then locate the first intermediate quantity that departs from the baseline invariant; downstream task error alone rarely identifies the cause.

## Recovery

Restore maximum-range/no-return handling, clamp evidence, and audit entropy plus false-free rate. Recovery is complete only when the original default inputs and diagnostic signature return within the independent-reference tolerance. A different setting that happens to look better is mitigation, not recovery. Preserve the fault, detection, decision, and recovery sequence as separate evidence.

## Alternative and limiting cases

- At hit probability one half, a measurement adds no occupancy information.
- With zero beams, the prior map remains unchanged.
