...
The interactive implementation retains **explicit shifted paths, signed gains, overlap, two sweeps, and three equivalent convolution formulations**. Change **middle echo delay** and **third signed gain** one at a time; the parameter-sweep plot supplies the pinned comparison cases. The broken toggle does exactly this: Overwrite rather than add samples where shifted signed echoes overlap. Recovery is equally explicit: Accumulate every path so manual, explicit-sum, and NumPy convolution outputs agree.
