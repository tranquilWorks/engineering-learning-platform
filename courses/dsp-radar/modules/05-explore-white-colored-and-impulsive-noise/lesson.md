...
The interactive implementation retains **seeded noise families, equal-RMS normalization, crest/correlation/power/tone diagnostics, and two sweeps**. Change **colored-noise memory** and **interferer offset** one at a time; the parameter-sweep plot supplies the pinned comparison cases. The broken toggle does exactly this: Compare raw white, colored, narrowband, and impulsive records at unequal RMS. Recovery is equally explicit: Mean-center and renormalize every record to the same target RMS.

The implementation is deterministic, bounded NumPy software. It deliberately omits MATLAB interactive figure-window behavior; no MATLAB runtime equivalence, browser/accessibility result, learner validation, audio-device result, or hardware claim is made.
