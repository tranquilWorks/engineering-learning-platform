...
def run(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "metrics": [
            {"id": "primary", "label": 'Clutter Ridge Slope', "value": primary, "unit": "× baseline", "emphasis": "primary"},
        ...
        "plots": {
            ...
            ], "layout": _layout(title + " — model view", 'look angle (deg)', 'normalized power'), "config": {"responsive": True, "displaylogo": False}},
            "response_view": {"data": [
                {"type": "scatter", "mode": "lines", "name": "response", "x": response_axis, "y": np.asarray(response).real},
            ], "layout": _layout(title + " — response view", 'look angle (deg)', 'normalized power'), "config": {"responsive": True, "displaylogo": False}},
            ...
            ], "layout": _layout("Intentional assumption failure", 'look angle (deg)', 'normalized power'), "config": {"responsive": True, "displaylogo": False}},
        },
        "explanations": {
            "observation": f"The {title} model uses a bounded deterministic array and space-time processing experiment. Primary scale={primary:.2f} and secondary stress={secondary:.2f} remain independently controllable.",
