...
The interactive implementation retains **FIR/IIR magnitude, group delay, impulse, step, pulse, signal/noise, arithmetic, tap, and Q comparisons**. Change **FIR tap count** and **IIR quality factor** one at a time; the parameter-sweep plot supplies the pinned comparison cases. The broken toggle does exactly this: Move a resonator pole radius to 1.02 so its impulse envelope grows. Recovery is equally explicit: Move the radius to 0.98 and verify a decaying response.

The implementation is deterministic, bounded NumPy software. It deliberately omits MATLAB filter-design and figure-window shortcuts; no MATLAB runtime equivalence, browser/accessibility result, learner validation, audio-device result, or hardware claim is made.
