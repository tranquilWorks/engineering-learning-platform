...
The model evaluates y=b0+bA A+bT T+bAT AT; b=(X^T X)^-1 X^T y; rank(X)=4. Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep aero_scale alone to its declared low case, then restore it and sweep tire_scale alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. A balanced factorial must separate main effects from interaction and expose residual error. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
