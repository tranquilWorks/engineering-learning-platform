...
The model evaluates L=sum ds; kappa=(x'y''-y'x'')/(x'^2+y'^2)^(3/2); integral kappa ds=2 pi. Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep major_radius_m alone to its declared low case, then restore it and sweep point_count alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. Closed geometry must satisfy position, heading, and signed-curvature closure. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
