...
The model evaluates r_line=r_center+n d; kappa_line=d psi/ds; J=sum ds/v(kappa)+lambda sum Delta d^2. Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep maximum_offset_m alone to its declared low case, then restore it and sweep smoothness_weight alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. A faster line must remain within track bounds while respecting the speed-dependent envelope. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
