...
The model evaluates t_setup=t_base-Delta t; u_t=f_u t; pass=all(subsystem ledgers). Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep setup_delta alone to its declared low case, then restore it and sweep uncertainty_fraction alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. All eleven subsystem-through-prediction requirements must reconcile and be signed. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
