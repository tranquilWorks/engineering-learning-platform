...
The model evaluates coverage=sum signed/9; r_val=y_val-yhat_val; pass=all(requirements). Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep validation_fraction alone to its declared low case, then restore it and sweep fault_severity alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. All nine replay-through-recovery requirements must be independently signed. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
