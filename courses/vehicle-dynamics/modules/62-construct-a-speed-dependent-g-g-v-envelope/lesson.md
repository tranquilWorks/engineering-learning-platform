...
The model evaluates Fz=mg+0.5 rho Cl A v^2; Fx=min(Ftire,(P/v-Fdrag)); (ax/axmax)^2+(ay/aymax)^2<=1. Every reported scalar belongs to a named signature so the independent oracle can reproduce the calculation without importing production code.
...
Sweep speed_mps alone to its declared low case, then restore it and sweep friction_scale alone to its high case. This isolates cause and effect while all other assumptions remain fixed.
...
Check the low and high control bounds. The g-g-v boundary must reconcile tire, propulsion, braking, aerodynamic, and coupled-friction limits. Finite values alone are insufficient; the signed residual or requirement ledger is the invariant.
